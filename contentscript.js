...
chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
    ...
    if (document.getElementsByClassName("venue-name").length > 0) {
        let eventName = document.getElementsByClassName("venue-name")[0].textContent;
        eventName = eventName.trim();
    } else if (document.getElementsByClassName("event-header").length > 0) {
        let eventName = document.getElementsByClassName("event-header")[0].firstChild.firstChild.textContent;
    } else if (document.getElementsByClassName("title avb").length > 0) {
        let eventName = document.getElementsByClassName("title avb")[0].childNodes[0].textContent + " vs. " + document.getElementsByClassName("title avb")[0].childNodes[4].textContent;
    } else if (document.getElementsByClassName("title runners").length > 0) {
        let eventName = document.getElementsByClassName("title runners")[0].getElementsByClassName("runner-name")[0].textContent + " vs. " + document.getElementsByClassName("title runners")[0].getElementsByClassName("runner-name")[1].textContent;
    }
...
function hasChanged(target, type) {
    ...
    for (let i = 0; i < id_list.length; i++) {
        ...
        if (id_list.item(i) !== "clicked_element_class" && id_list.item(i) !== "clicked_element" && id_list.item(i) !== "bet-button-price") {
            chrome.runtime.sendMessage({
                from: 'content',
                subject: 'odds_update',
                odds: odds,
...
function setUpTempData(element_div) {
    ...
    if (actual_runner.getElementsByClassName("horse-racing-silk").length > 0) {
        let imageurl = actual_runner.getElementsByClassName("horse-racing-silk")[0].src;
    } else {
        let imageurl = "none";
    }

    let runnerName = actual_runner.getElementsByClassName("runner-name")[0].firstChild.textContent;
    let odds = element_div.parentNode.firstChild.textContent;
    let backLay;
